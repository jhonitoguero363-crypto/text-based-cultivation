...
import { PILL_MARKET_RARE_CATALOG } from './pill-market-rare'

export interface CatalogPill {
  id: string
  name: string
  grade: string
  /** 效果方向八类：灵根丹 / 悟性丹 / 修为丹 / 聚灵丹 / 战斗丹 / 突破丹 / 保命丹 / 特殊丹 */
  type: string
  effect: string
  special: string
  story: string
...
export function getPillByName(name: string) {
  return (
    PILL_SHOP_CATALOG.find((item) => item.name === name) ||
    PILL_MARKET_RARE_CATALOG.find((item) => item.name === name) ||
    null
  )
}

/** 丹阁 + 坊市稀有 全量目录（图鉴 / 查价） */
export function getAllPills(): CatalogPill[] {
  return [...PILL_SHOP_CATALOG, ...PILL_MARKET_RARE_CATALOG]
}
